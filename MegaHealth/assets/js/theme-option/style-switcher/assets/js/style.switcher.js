jQuery(function($) { 
	var styleSwitcher = {
		...
		// Demo click function
			$(document).on('click','#design .layout-column',function() {
			$('#design .layout-style .layout-column').removeClass('radio-active');
			$(this).addClass('radio-active'); 
			var data = $(this).attr('data-layout');
	
	 	$('.design-layout div').slideUp('normal');
	
			$(this).parents('.layout-style').next('.design-layout').find('.'+ data).slideDown('normal');
	...
	//$('.layout-column').on('click',function(){
		//$()
		//var data = $(this).attr('data-layout');
		//$('.'+ data)
	///})
