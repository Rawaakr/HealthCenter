						...
						<form method="post" action="controller/ajoutRendezvous.php" name="contactForm" novalidate id="contact">
							...
							<div class="row mb-28">
								<div class="col-sm-6">
									...
									<input class="input-box" type="date" name="dateNAiss" required="" data-ng-model="formData.email" data-ng-pattern = "/^\w+@[a-zA-Z_]+?\.[a-zA-Z]{2,3}$/" data-ng-class="{'error' : contactForm.email.$error.pattern && contactForm.$blured || contactForm.email.$invalid && successStatus}">
								</div>
							</div>
							<input class="btn" type="submit" value="créer"	data-ng-click = "submitcotactusForm(contactForm.$invalid)">
						</form>
					</div>
